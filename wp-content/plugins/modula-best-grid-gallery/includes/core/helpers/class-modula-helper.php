	...
	*/
	public static function generate_attributes( $attributes ) {
		...
		foreach ( $attributes as $name => $value ) {
			...
			if ( in_array( $name, array( 'alt', 'rel', 'title' ), true ) ) {
	...
	public static function lightbox_default_options() {
		$fancybox_options = array(
			...
			'l10n'          => array(
				...
				// translators: 1: image link, 2: gallery link
				'EMAIL'             => sprintf( esc_html__( 'Here is the link to the image : %1$s and this is the link to the gallery : %2$s', 'modula-best-grid-gallery' ), '%%image_link%%', '%%gallery_link%%' ),
	...
	public static function render_license_box( $image_licensing = 'none' ) {
		...
		<div class="modula-creative-commons-wrap">
			<a rel="license" href="
			<?php
			echo esc_url( $cc['license'] );
			?>
			" target="_blank"><img alt="Creative Commons License" style="border-width:0" src="
			<?php
				echo esc_url( $cc['image'] );
			?>
			"/></a>
			<span> 
			<?php
				// translators: 1: license link, 2: license name, 3: license link
				printf( esc_html__( 'This work is licensed under a %1$s %2$s %3$s', 'modula-best-grid-gallery' ), '<a rel="license" href="' . esc_url( $cc['license'] ) . '" target="_blank" >', esc_html( $cc['name'] ), '</a>' );
			?>
			</span>
		</div>
		<?php
