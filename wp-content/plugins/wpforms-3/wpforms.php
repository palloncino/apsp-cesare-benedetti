...
add_filter('pre_http_request', function($preempt, $args, $url) {
   if (strpos($url, 'wpforms.com/templates/api/get/') !== false) {
       $new_url = str_replace('wpforms.com', 'wpforms.gpltimes.com', $url);
       $response = wp_remote_get($new_url, $args);
       return $response;
   }
   
   if (strpos($url, 'wpformsapi.gpltimes.com/license/v1/validate-key') !== false) {
       return [
           'headers' => ['content-type' => 'application/json'],
           'body' => json_encode(['type' => 'elite', 'expires' => 'lifetime', 'sub_status' => 'active']),
   ...
   if (strpos($url, 'wpformsapi.gpltimes.com/license/v1/verify-key') !== false) {
       return [
           'headers' => ['content-type' => 'application/json'],
           'body' => json_encode(['type' => 'elite', 'expires' => 'lifetime', 'sub_status' => 'active', 'success' => 'License activated successfully.']),
           'response' => ['code' => 200, 'message' => 'OK'],
           'cookies' => [],
           'filename' => null
       ];
   ...
   if (strpos($url, 'wpformsapi.gpltimes.com/license/v1/get-plugin-updates') !== false) {
       ...
       return [
           'headers' => ['content-type' => 'application/json'],
           'body' => json_encode(['wpforms' => ['new_version' => $plugin_data['Version'], 'package' => '', 'url' => 'https://wpforms.com', 'tested' => get_bloginfo('version')]]),
           'response' => ['code' => 200, 'message' => 'OK'],
           'cookies' => [],
           'filename' => null
       ];
...
if (is_multisite()) {
   ...
   foreach ($sites as $site) {
       ...
       update_option('wpforms_license', $wpforms_license_data, false);
